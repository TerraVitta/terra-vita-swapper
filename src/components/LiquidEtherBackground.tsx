...
`;
    ...
    const applyOptionsFromProps = () => {
      ...
      // Update palette texture on theme change
      if ((webglRef.current as any)?.output?.output?.material?.uniforms) {
        const uni = ((webglRef.current as any).output.output.material as THREE.RawShaderMaterial).uniforms;
        if (uni && uni.palette) uni.palette.value = paletteTex;
      }
  ...
  }, [BFECC, cursorSize, dt, isBounce, isViscous, iterationsPoisson, iterationsViscous, mouseForce, resolution, viscous, colors, autoDemo, autoSpeed, autoIntensity, takeoverDuration, autoResumeDelay, autoRampDuration, theme]);

  // Update palette and background alpha on theme change
  useEffect(() => {
    if (!webglRef.current) return;
    const webgl = webglRef.current;
    const sim = webgl.output?.simulation;
    const outputMesh = (webgl.output as any)?.output;
    ...
    if (material && material.uniforms) {
      ...
      if (material.uniforms.bgColor && material.uniforms.bgColor.value) {
        material.uniforms.bgColor.value.set(0, 0, 0, bgAlpha);
      }
